...
6. **`categoriaId`/`subcategoriaId` sin backend real todavía** _(resuelto v1.4.0)_: SPEC-016 los documentaba como "TODO — sin FK, sin módulo de Categorías". `api-pos` implementó el módulo real (SPEC-020) — el mock se retiró; ver REQ-U20/U49 a U52.
...
├──────────────────────────────────────────────────────────────┤
│ * Campos requeridos                    [Cancelar] [Guardar]   │
└──────────────────────────────────────────────────────────────┘
...
- **REQ-U9**: El área superior de cualquier vista con `DataTable` DEBE seguir el mismo patrón: buscador siempre visible a la izquierda, botón "Filtros" junto a él (abre popover con el resto de campos), y la acción primaria de la vista (ej. "Agregar") a la derecha.
...
- **REQ-U20** _(revisado v1.4.0)_: `categoriaId`/`subcategoriaId` (sección "Clasificación" de Crear, y tab "Información general" de Ver/Editar) DEBEN capturarse con un `react-select` (`components/Select`, REQ-U35) poblado con datos **reales** de `GET /api/v1/categorias/selector` (api-pos SPEC-020) — el mock estático se retiró, el módulo de Categorías ya existe en el backend. Nunca un input de texto libre para un UUID.
...
- **REQ-U28**: El tab "Costos y precio" DEBE mostrar, en este orden: formulario de ajuste (costoEstimado, costoPromedio, precioVenta, margenDeseadoPorcentaje — los 4 vía `MaskedInput`/`react-imask`, REQ-U37) con botón "Guardar" (`PATCH /:id/costo` — verbo solo, ver SPEC-001 §Contenido y copy de botones), seguido de la gráfica y tabla de historial.
...
- **REQ-U43** _(nuevo v1.2.0)_: Todo botón del módulo de Productos DEBE usar `size="sm"` de `components/Button` — tamaño único estandarizado para toda la feature (antes mezclaba `sm` y el default `md` sin criterio, hallazgo de la revisión visual v1.1.0). El contenido (ícono + verbo en infinitivo, qué acciones llevan ícono y cuáles no) sigue la regla transversal de SPEC-001 §Contenido y copy de botones — no se repite aquí.
...
- **REQ-U45** _(nuevo v1.2.0)_: Cuando el listado no tenga ningún producto registrado (REQ-X2), la toolbar DEBE ocultar el botón "Agregar" — el CTA del estado vacío (mismo label, "Agregar") es el único punto de entrada en ese momento. Nunca mostrar dos botones con el mismo objetivo a la vez (hallazgo de la revisión visual v1.1.0).
...
- **REQ-U47** _(nuevo v1.3.0, actualizado v1.5.0)_: Los botones equivalentes de creación ("Agregar" — listado, toolbar y estado vacío) y de guardado ("Guardar" — Crear, Información general, Ajustar costo) DEBEN compartir el mismo ícono (`components/icons/PlusIcon` y `components/icons/SaveIcon` respectivamente — movidos de `features/productos/components/icons.tsx` a `components/icons/` en v1.5.0 porque Categorías también los necesita, ver §Dependencias). Los botones de acción secundaria/dismissive ("Cancelar", "Limpiar", "Volver") NO llevan ícono. Regla completa (incl. por qué, y la prohibición de frases verbo+objeto como "Guardar producto") ahora vive en SPEC-001 §Contenido y copy de botones — este REQ solo confirma que Productos la cumple.
...
- **REQ-U49** _(nuevo v1.4.0)_: Los selectores "Categoría" y "Subcategoría" DEBEN incluir un botón "+" icon-only contiguo (mismo `size="sm"`, `aria-label` descriptivo — REQ-U49 no exige label visible ahí: es una acción secundaria de espacio reducido junto a un campo, no una acción primaria de la vista) que abre un modal de registro rápido (`CategoriaQuickCreateModal`/`SubcategoriaQuickCreateModal`, `features/categorias/`). El botón de Subcategoría DEBE estar deshabilitado mientras no haya `categoriaId` elegido — mismo gate que el propio select (REQ-U46).
- **REQ-U50** _(nuevo v1.4.0)_: Al crear una categoría/subcategoría desde el modal de registro rápido, el sistema DEBE: (a) invalidar la query de selector de categorías (`['categorias']`) para que cualquier select montado en ese momento vea la opción nueva, (b) autoseleccionar la categoría/subcategoría recién creada en el campo que disparó el modal — el usuario no debe volver a buscarla manualmente en la lista que acaba de poblar.
- **REQ-U51** _(nuevo v1.4.0)_: `SubcategoriaQuickCreateModal` DEBE incluir un selector "Categoría padre" (a diferencia de `CategoriaQuickCreateModal`, que crea siempre una categoría raíz sin ese campo). Cuando el modal recibe `defaultPadreId` (caso real de uso: el botón "+" de `SubcategoriaSelect`, que solo se habilita con una categoría ya elegida en el formulario que lo contiene), ese selector DEBE quedar precargado y deshabilitado — nunca editable en ese contexto, para no crear una subcategoría cuyo padre real no coincida con la categoría que el formulario de origen espera seleccionar después.
- **REQ-U52** _(nuevo v1.4.0)_: Todo modal disparado por un botón puntual (ej. los "+" de REQ-U49) DEBE recibir el ref de ese botón como `originRef` de `components/Modal`, de forma que la entrada anime desde la posición del botón hacia el centro (`scale`+`translate`+`fade`, ~200ms ease-out, respeta `prefers-reduced-motion`) en vez de crecer desde el centro puro — refuerza la relación causa-efecto sin intentar un morph real (FLIP) entre un botón pequeño y una modal ancha con formulario, que se vería distorsionado por la diferencia de forma/aspecto entre origen y destino.
- **REQ-U53** _(nuevo v1.5.0)_: `components/Modal` DEBE animar también su salida (`scale`+`translate`+`fade` de vuelta hacia el `originRef`, o hacia el centro si no hay uno) — nunca desaparecer de golpe en cuanto `isOpen` pasa a `false`. La duración de salida DEBE ser menor a la de entrada (~70%, regla `exit-faster-than-enter`) — se siente más responsiva al cerrar.
- **REQ-U54** _(nuevo v1.5.0)_: Todo consumidor de un modal cuyo contenido dependa de datos que el propio cierre podría limpiar (ej. "Eliminar producto", que muestra el nombre del producto objetivo) DEBE renderizar el componente wrapper **siempre** — nunca `{condición && <Modal.../>}`. Si el padre desmonta el wrapper completo al cerrar, React lo retira del árbol antes de que la animación de salida de REQ-U53 pueda correr (el `Modal` interno nunca llega a reproducirla). El wrapper controla su propio `isOpen` a partir de la condición, y cachea internamente el último valor no nulo de los datos que muestra (ej. `EliminarProductoModal` cachea `producto` en un `useState` propio) para que el contenido no desaparezca antes que la animación.
- **REQ-U55** _(nuevo v1.5.0)_: Ningún footer de modal o de card de formulario lleva `border-t` como separador visual — el espaciado (`gap`/`pt`) es suficiente y una línea divisoria ahí se percibe como ruido (hallazgo de revisión visual v1.5.0). Aplica a `CategoriaQuickCreateModal`, `SubcategoriaQuickCreateModal`, `EliminarProductoModal`, `ProductoCrearForm`, `ProductoInfoGeneralForm`, `FilterPopover` (mismo patrón de footer, aunque no es una card de formulario ni un modal).
...
- **REQ-E3**: Cuando el usuario haga clic en "Agregar", el sistema DEBE navegar a `/productos/nuevo`.
...
- **REQ-O2** _(resuelto v1.4.0 — ya no es "optional", ver REQ-U20)_: El selector mock de `categoriaId`/`subcategoriaId` fue reemplazado por `react-select` con carga real contra `GET /categorias/selector` — sin cambios en la forma del campo del formulario (`categoriaId: string`), como esta spec anticipaba.
...
- **REQ-X2**: Si el total de productos de la empresa es 0 sin ningún filtro aplicado, el sistema DEBE ocultar la tabla y mostrar un estado vacío dedicado ("Aún no has registrado productos" + botón "Agregar") — nunca el mismo estado vacío que REQ-X3.
- **REQ-X3**: Si hay filtros o búsqueda activos y el resultado es 0, el sistema DEBE mostrar un estado vacío distinto ("No se encontraron productos con estos filtros" + botón "Limpiar") — nunca reutilizar el CTA "Agregar" de REQ-X2.
- **REQ-X4**: Si el usuario navega directo a `/productos/:id` con un `id` inexistente o de otra empresa (`ERR_PRODUCTO_NOT_FOUND`, 404), el sistema DEBE mostrar un estado de error dedicado ("Producto no encontrado") con acción "Volver" — nunca un formulario vacío ni una excepción sin capturar.
- **REQ-X5**: Si un usuario sin rol `superadmin`/`admin` navega directo a `/productos/nuevo` por URL, `ProductoCrearPage` DEBE redirigir a `/productos` sin renderizar el formulario — el control de acceso no puede depender únicamente de ocultar el botón "Agregar" (REQ-U24 aplicado también aquí).
...
- **Depende de**: `api-pos` SPEC-020 (Módulo de Categorías, v1.4.0) — `GET /categorias/selector` alimenta `CategoriaSelect`/`SubcategoriaSelect`; `POST /categorias` alimenta los modales de registro rápido. `categoriaId` pasó a ser requerido también en el backend (`crearProductoSchema`, confirmado en el código real de `api-pos`), coincide con REQ-U46 (que ya lo exigía a nivel de UX antes de que el backend lo exigiera).
...
- **Riesgo documentado (resuelto v1.4.0)**: `categoriaId`/`subcategoriaId` dependían de un selector mock (REQ-U20) hasta que existiera el módulo real de Categorías en `api-pos` — ya existe (SPEC-020), REQ-O2 se marca resuelto.
- **Riesgo documentado (nuevo v1.4.0)**: el rol-gate de Categorías en `api-pos` (SPEC-020) también usa `verificarRoles` estático (no el catálogo dinámico de permisos, SPEC-003) — el backend está migrando de permisos estáticos a dinámicos en general; si esa migración llega a Productos/Categorías más adelante, `PRODUCTO_ROLES_ESCRITURA` y el equivalente en `CATEGORIA_ROLES_ESCRITURA` (arrays hardcodeados comparados contra `data.role`) deben migrar a `tieneAccion(data?.modulos, '<clave>')`, mismo patrón que ya usan ventas/inventario/clientes. No es bloqueante hoy porque ambos módulos backend confirman explícitamente estar fuera de alcance de esa migración por ahora.

## Cambios

- v1.5.0 (2026-08-04): Hallazgos de revisión visual: contenido de botones, `border-t` de footers y animación de salida de `Modal`. (1) **SPEC-001 actualizada** — nueva sección "Contenido y copy de botones" (regla transversal: ícono+label en acciones primarias, sin ícono en secundarias/dismissive, verbo en infinitivo sin objeto — nunca "Guardar producto"/"Crear categoría"/"Cancelar edición"). Todos los labels de Productos/Categorías se renombraron para cumplirla: "Nuevo producto"/"Crear producto" → "Agregar"; "Crear categoría"/"Crear subcategoría"/"Guardar ajuste" → "Guardar"; "Limpiar filtros" → "Limpiar"; "Volver al listado" → "Volver"; "Cancelar edición" → "Cancelar". (2) `PlusIcon`/`SaveIcon` se mueven de `features/productos/components/icons.tsx` a `components/icons/` (genérico) — Categorías también los necesita, y una feature no debe importar archivos internos de otra. (3) **REQ-U55** (nuevo) — se quita `border-t` de todos los footers de modal/card de formulario/`FilterPopover`. (4) **REQ-U53** (nuevo) — `Modal` ahora anima también su salida (antes solo la entrada; desaparecía de golpe). (5) **REQ-U54** (nuevo, el hallazgo más importante de esta revisión) — al implementar la salida se descubrió que no alcanzaba con animar `Modal`: los consumidores que montaban el wrapper condicionalmente (`EliminarProductoModal`, `CategoriaQuickCreateModal`, `SubcategoriaQuickCreateModal`) desmontaban todo el árbol de React antes de que la animación pudiera correr. Se migraron los tres a "siempre renderizado, `isOpen` como prop" — `EliminarProductoModal` ahora cachea internamente el último `producto` no nulo (`useState`) para que el nombre no desaparezca a mitad de la animación; los modales de registro rápido resetean su formulario en un `useEffect` atado a `isOpen` (ya no pueden confiar en un mount fresco para eso).
- v1.4.0 (2026-08-04): `api-pos` implementó el módulo real de Categorías (SPEC-020) — se retira el mock. (1) **Nueva feature `features/categorias/`** (types, constants, schema, service, hooks `useCategoriasSelector`/`useCrearCategoria`) — reutilizable fuera de Productos si algún otro módulo llega a asignar categorías. (2) `lib/applyApiError.ts` (nuevo) — se extrajo la lógica genérica de `applyProductoApiError` (antes solo en Productos) para reutilizarla en Categorías sin duplicar la estrategia de dos niveles (`error.details` vs. tabla estática); `applyProductoApiError` queda como wrapper delgado. (3) **REQ-U49/U50** (nuevos) — botón "+" junto a Categoría/Subcategoría que abre un modal de registro rápido; autoselecciona lo creado e invalida el selector. (4) **REQ-U51** (nuevo) — `SubcategoriaQuickCreateModal` incluye selector de categoría padre, bloqueado cuando llega precargado desde el contexto real de uso (evita crear una subcategoría con padre distinto al que el formulario de origen espera). (5) **REQ-U52** (nuevo) — animación origin-aware en `components/Modal` (`originRef`, scale+translate+fade desde el botón que disparó la modal, ~200ms ease-out, respeta `prefers-reduced-motion`); decisión tomada tras descartar un morph real (FLIP) por el riesgo de distorsión entre un botón pequeño y una modal ancha. De paso se corrigió `bg-background-secondary` → `bg-background` en la card de Modal (mismo bug de REQ-U44) y un hallazgo de lint (`react-hooks/set-state-in-effect`, resuelto con el patrón de ajuste de estado durante el render en vez de un efecto). (6) Se retiran `CATEGORIAS_MOCK`/`CATEGORIAS_PADRE_MOCK`/`getSubcategoriasDe` de `producto.constants.ts` (dead code). Riesgo nuevo documentado: Categorías (igual que Productos) usa `verificarRoles` estático en el backend, no el catálogo dinámico de permisos que el backend está migrando — si esa migración alcanza estos módulos, los arrays de roles hardcodeados en el front deben migrar a `tieneAccion()`, cambio localizado por diseño.
